...
CAMERA_OFFSET = 0.11 #0.13 #16  #0.06 # m from center car to camera
    ...
    if md is not None:
      ...
      try:
        if LaC is not None and LaC.angle_steers_des_mpc != 0.0:
          ...
          angle_error = 0.0
        if angle_error != 0.0:
          lateral_error = 0.5 * np.clip(v_ego * (LaC.steerActuatorDelay + 0.05) * math.tan(math.radians(angle_error)), -0.5, 0.5)
        else:
          lateral_error = 0.0
      except:
        lateral_error = 0.0
      ...
      speed_lane_width = interp(v_ego, [0., 31.], [2.7, 3.7]) #[3., 3.6])
